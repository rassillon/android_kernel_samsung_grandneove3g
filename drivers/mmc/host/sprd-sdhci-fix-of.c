...
static int sdcard_detect_state;
...
static const struct sprd_sdhci_host_fix sprd_sdhci_host_fix_base = {
	.quirks = SDHCI_QUIRK_NO_HISPD_BIT | SDHCI_QUIRK_DATA_TIMEOUT_USES_SDCLK | SDHCI_QUIRK_BROKEN_CARD_DETECTION | SDHCI_QUIRK_CLOCK_BEFORE_RESET,
	.quirks2 = SDHCI_QUIRK2_PRESET_VALUE_BROKEN | SDHCI_QUIRK2_TIMEOUT_DIVIDE | SDHCI_QUIRK2_USE_MAX_DISCARD_SIZE,
};
...
static void sprd_sdhci_host_of_parse(struct platform_device *pdev, struct sdhci_host *host) {
	...
	if (of_find_property(np, "sdhci-cap-hw-reset", &len))
		host_pdata->caps |= MMC_CAP_HW_RESET;
	if (of_find_property(np, "sdhci-cap-1v8-ddr", &len))
		host_pdata->caps |= MMC_CAP_1_8V_DDR;
	if (of_find_property(np, "sdhci-cap2-hc-erase-sz", &len))
		host_pdata->caps2 |= MMC_CAP2_HC_ERASE_SZ;
	if (of_find_property(np, "sdhci-cap2-cache-ctrl", &len))
		host_pdata->caps2 |= MMC_CAP2_CACHE_CTRL;
...
static int sprd_sdhci_host_probe(struct platform_device *pdev)
{
	...
	/* Workaround : Black screen from SD card removal during suspend
	   Disable MMC_PM_IGNORE_PM_NOTIFY feature to SD card */
	if (pdev->id != SDC_SLAVE_SD)
